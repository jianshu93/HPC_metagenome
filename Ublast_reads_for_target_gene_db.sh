...
if [[ "$1" == "" || "$1" == "-h" ]] ; then
   ...
   Note: The fa file should be the SAME named to the folder's name.
...
target_genes_dir=$(readlink -f $1) ;
target_gene_name=$(basename ${target_genes_dir})
...
if [ ! -f "$target_genes_dir/${target_gene_name}_length" ] ; then
  ...
  for i in $reads_dir/*.fastq ; do
    ...
    if [[ $k -ge $MAX ]] ; then
      ...
      # Launch job
      jids[$k]=$(sbatch --export="READ_DIR=${reads_dir},SAMPLE=$b,TARGET_DIR=${target_genes_dir},TARGET_NAME=${target_gene_name}" -J "ublast-$b_n" -o "out-$b_n.log" $pac/run_ublast.slurm | grep .) ;
